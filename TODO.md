...
- [x] **Volatility-aware stop (2026-05-29)**: `PositionEvent` gained an optional `atr_pct` hint (recent ATR as % of price; omitted when the producer doesn't track it). `GuidanceThresholds::widen_for_volatility(atr_pct, atr_multiplier)` loosens the stop to sit outside an ATR-sized noise band (`atr_multiplier × atr_pct`), reusing the optimizer's own `atr_multiplier` (exposed as `DEFAULT_ATR_MULTIPLIER` for the un-optimized fallback). Take-profit left untouched — it's a target, not noise protection. Handler widens before computing guidance; logs `atr_pct` + effective `stop_loss_ratio`.
- [x] **Amygdala fear → guidance (2026-05-29)**: `JanusState` gained `current_threat: Option<f64>` (0–1, snapshot, mirrors `current_regime`; kept a plain `f64` so janus-core takes no neuromorphic dep). Forward's signal-rx loop captures `signal.metadata["fear"]` opportunistically — same zero-API-surface channel as `regime`. `compute_guidance` now takes `fear: Option<f64>` and applies a graduated policy: high fear (≥`FEAR_EXIT_LEVEL` 0.8) → Exit; elevated (≥`FEAR_ELEVATED_LEVEL` 0.5) → Reduce a winner / `tighten_stop_for_fear` on a loser (stop scales to 25% of width at the top of the band, never to break-even). Crisis regime still outranks fear. Producer side (emitting `"fear"` from the real fear network) is an fks/forward-pipeline follow-up, like `regime` is today.
