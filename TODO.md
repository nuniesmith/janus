...
- [ ] **Close the learning loop.** The outcome rows exist; still missing: (a) JanusAI-side compaction of `janus_position_outcomes` + `janus_position_events` into `janus_memories` (fks repo), and (b) feeding realized outcomes back into the strategy-affinity tracker so guidance adapts over time.
