...
- [x] **Dependency vuln scanning in CI (2026-05-31).** `.github/workflows/security.yml` runs `cargo-audit` (RUSTSEC) on dependency-manifest changes, weekly, and on demand. Non-blocking (`continue-on-error`) for now so a finding won't red-line `main` while the dep backlog (e.g. the tonic split) is triaged — flip to a hard gate once clean. Trivy filesystem scan still TODO.
