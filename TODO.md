...
- [x] **Redis bootstrap load (2026-05-25)**: `janus-api::start_module` now best-effort `HGETALL`s `fks:{instance}:optimized_params` at boot so production guidance actually uses tuned thresholds. Missing/empty hash or unreachable Redis → warn + fall back to defaults.
- [ ] Subscribe to `fks:{instance}:param_updates` from janus-api so tuned thresholds refresh without a restart (services/forward already subscribes for its own appliers; sharing one `ParamManager` via `JanusState` would dedupe the Redis traffic — future cleanup).
