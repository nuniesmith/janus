...
mod tests {
    ...
    async fn shared_workers() {
        // Skip unless REDIS_RESP3=1 is explicitly set (RESP3 protocol not available in standard Redis)
        if env::var("REDIS_RESP3").as_deref() != Ok("1") {
            eprintln!(
                "Skipping: REDIS_RESP3=1 not set (requires Redis with RESP3 protocol support)"
            );
            return;
        }
        let url = env::var("REDIS_URL").unwrap();
        let info: redis::ConnectionInfo = url.parse().unwrap();
        let resp3_settings = info
            .redis_settings()
            .clone()
            .set_protocol(redis::ProtocolVersion::RESP3);
        let info = info.set_redis_settings(resp3_settings);
